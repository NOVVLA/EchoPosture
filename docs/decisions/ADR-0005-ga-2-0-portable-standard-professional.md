...
- 范围：在 `ga-2.0.0` 同一标签下追加半便携图形安装器及三个官方程序分片；`docs/RELEASE.md` §4b 的解除条件

## 决策

在已发布的 `ga-2.0.0` 标签下，追加半便携图形安装器和三个官方程序分片。它与 §4a 的纯源码包并存，
不新建标签、不覆盖已发布的源码资产。原始半便携 ZIP 仍保留为固定的重组审计基线；由于 GitHub
Release 单资产 2 GiB 限制，它不作为单个 Release 资产上传。

追加的交付物是：

- `EchoPosture-GA-2.0.0-semi-portable-setup.exe`；
- `EchoPosture-GA-2.0.0-semi-portable-win-x64.zip.001`、`.002`、`.003`；
- 与分片对应的公开 manifest 和 SHA-256SUMS 文件。

安装器源码提交与应用来源提交分别写入 manifest，便于审计两个构建来源而不移动已发布标签。
...
`ga-2.0.0` 标签继续精确指向 `371fb71`（源码包所对应的提交）；安装器源码提交为
`3cfd5533a2b53dcf8711f4f2e6d79249bb9732bf`，并在公开 manifest 中与应用来源提交
`371fb71b2bc20834608f1edd59d1de4fd88b3126` 分开记录。原始 ZIP 的大小和摘要只作为三片重组
审计基线，不代表一个可直接下载的单体 Release 资产。
