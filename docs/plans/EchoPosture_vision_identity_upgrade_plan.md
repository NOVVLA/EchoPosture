...
截至 2026-08-11，P3/P4 的统一观测契约、兼容后端、目标状态机、无稳定 ID 交叉回放、歧义保护和 Worker 校准门控已有确定性代码与测试覆盖；`EP-TRACK-006` 仍为 `NEXT`，必须在真实多人后端接入前关闭关联复杂度风险。P5 已获授权进入模型无关基础实现，但 `EP-ID-008` 运行时启用门控、模型权重集成、许可、真实录像和性能证据仍保持未完成状态。
