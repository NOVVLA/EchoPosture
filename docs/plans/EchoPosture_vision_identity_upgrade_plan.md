...
2026-08-13 复核：上表是方案编制时的旧基线，不再代表当前代码。兼容后端现已接入统一人物观测、`TargetManager`、目标锁定/歧义状态、双锚点校准和科学姿态分析器；标准模式与专业模式仍只有模式契约和 Debug UI 入口，没有真实姿态推理后端。当前跟踪活动量仅来自目标人体框的归一化中心平移速度与尺度变化速度，不能证明用户完全没有移动。
...
| 标准模式（默认） | YOLO26n-pose；首版 Ultralytics PyTorch CPU，后续用实测决定是否导出 ONNX | 本轮明确跳过人脸处理 | 本轮不加载身份模型、不生成模板或 embedding | 普通现代 Windows 电脑 | 先完成 Debug UI 姿态链路，再接正式 EXE |
...
- [ ] **EP-STANDARD-004**：人脸裁剪、FaceMesh 和身份相关处理按用户要求移出本轮，等待单独计划；不是当前姿态-only 标准模式的完成条件。
...
| P5：本地人脸验证 | `PAUSED` | 已有三态异步验证等模型无关基础；其余模型、隐私和运行时门控工作等待用户单独计划 | `EP-ID-001` 至 `EP-ID-008` | 用户提交新的人脸计划后重新确认边界 | 同一测试集 A/B、误接受/误拒绝数据、关闭路径零模型加载/零验证请求证据、线程/隐私审计 |
| P6：标准模式 | `IN_PROGRESS` | YOLO26n-pose 本地权重原型、17 点统一输出、姿态-only 校准、有界多人关联和兼容回退；人脸裁剪延期 | `EP-STANDARD-001` 至 `EP-STANDARD-006` | P3/P4 契约稳定；项目接受 AGPLv3；权重只作本地开发输入 | fake 推理契约、CPU 关联 P50/P95、真实权重合成空帧验证与回退测试；正式 EXE 后接 |
...
截至 2026-08-14，P3/P4 的统一观测契约、兼容后端、目标状态机、无稳定 ID 交叉回放、歧义保护、Worker 校准门控和有界全局关联已有确定性代码与测试覆盖。P5 的 `EP-ID-001/002/006` 已完成模型无关基础与测试；其余人脸工作按用户要求暂停，等待单独计划。P6 已在 Debug UI 接入姿态-only YOLO26n-pose CPU 原型并完成 17 点统一观测和本地权重门控，但 ONNX 对照、输入尺寸/频率实验、正式 EXE 默认与回退接线仍未完成。Debug UI 的三模式选择和本地化不等于正式 `EP-UI-001/003` 完成；专业姿态后端也尚未实现。

移交规则：P1/P2 的模型和硬件证据门槛仍不得被跳过；人脸相关接口、模型选择和隐私验收等待用户的新计划，不得在标准姿态模式中自行恢复。每次交接必须在 `DEVELOPMENT_LOG.md` 记录提交号、验证命令、远端检查状态和剩余风险；`REVIEW`、`BLOCKED` 或未验证项不得被写成“已完成”。
