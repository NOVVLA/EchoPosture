...
- [ ] **EP-TRACK-006**：为全局一对一轨迹关联设置确定的候选人数/轨迹数上限和单帧计算预算；超过预算时必须采用有界回退并输出安全的歧义状态，不能让递归穷举拖慢视觉工作线程。补充高候选数、最坏候选矩阵和回退状态测试，并在接入真实多人后端前记录 P50/P95 延迟。此项状态为 `NEXT`，优先于 Phase 4 标准模式接入。
...
- [ ] **EP-UI-001**：增加兼容/标准/专业 Beta 三档选择和功能说明。Debug UI 已提前提供三档选择、实际后端和回退原因；正式产品设置 UI 及标准/专业后端仍未完成。
...
| P4：目标管理与当前模型状态机 | `IN_PROGRESS` | 轨迹关联、主用户锁定、遮挡/离开/重捕获、脸身关联、目标歧义安全状态和有界关联预算 | `EP-TRACK-001` 至 `EP-TRACK-006` | P3 可输出统一人物观测；先使用当前 MediaPipe 验证状态机；`EP-TRACK-006` 为接入真实多人后端前的 `NEXT` 门槛 | 录制回放矩阵、状态转移测试、无静默换人证据、高候选数有界回退和 P50/P95 延迟 |
| P5：本地人脸验证 | `IN_PROGRESS` | 三态异步验证、质量过滤、多帧聚合、事件触发、内存数据清理和真正控制处理行为的启用门控 | `EP-ID-001` 至 `EP-ID-008` | P2 许可证通过；P4 已稳定提供目标候选；隐私边界确认；默认启用或发行前完成 `EP-ID-008` | 同一测试集 A/B、误接受/误拒绝数据、关闭路径零模型加载/零验证请求证据、线程/隐私审计 |
...
截至 2026-08-13，P3/P4 的统一观测契约、兼容后端、目标状态机、无稳定 ID 交叉回放、歧义保护和 Worker 校准门控已有确定性代码与测试覆盖；`EP-VISION-012/013` 因其代码与确定性测试任务已完成而补勾。`EP-TRACK-006` 仍为 `NEXT`，必须在真实多人后端接入前关闭关联复杂度风险。P5 的 `EP-ID-001/002/006` 已完成模型无关基础与测试，`EP-ID-007` 只有会话内清理实现而缺少正式隐私证据，`EP-ID-008` 运行时启用门控、模型权重集成、许可、真实录像和性能证据仍保持未完成。Debug UI 的三模式选择和本地化属于提前完成部分，不等于正式 `EP-UI-001/003` 完成；标准和专业姿态后端均未实现。
