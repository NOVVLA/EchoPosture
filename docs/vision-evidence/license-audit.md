...
| --- | --- | --- | --- | --- | --- | --- |
| Ultralytics YOLO26n/l/x-pose | [Ultralytics repository](https://github.com/ultralytics/ultralytics)；[license guidance](https://www.ultralytics.com/license) | AGPL-3.0（GitHub API `license.spdx_id`） | 未冻结；计划只写模型族 | 官方许可页称 Ultralytics 训练模型默认受 AGPL-3.0 覆盖，但具体 YOLO26 权重文件、版本和 SHA-256 尚未登记 | 未从模型卡取得可用于本项目发行的训练数据条款 | `conditional`; 精确权重和发行方式确认前 `blocked` |
...
3. 明确 EchoPosture GPLv3 与 AGPLv3 组合发行时的完整对应源码、NOTICE 和用户获取方式。
