<#
.SYNOPSIS
    下载 EchoPosture 标准模式与专业 Beta 模式所需的 Ultralytics YOLO26 姿态权重，
    来源为官方 GitHub 发布地址。本脚本是 fetch_pose_models.ps1 的中文本地化版本，
    行为与英文版完全一致，仅界面文字为中文。

.DESCRIPTION
    GA-2.0 发行包不包含模型权重。Ultralytics YOLO26 权重采用 AGPL-3.0 许可证，
    由每位用户自行从上游发布方获取，本脚本不会镜像或转发这些文件。

    兼容模式不需要运行本脚本。标准模式与专业 Beta 模式则必须运行。

    每个文件在被接受前都会与预先固定的 SHA-256 值进行校验。

.PARAMETER Tier
    要获取的权重档位。
      Standard     - 仅 yolo26n-pose.pt（约 7.9 MB）。默认值。
      Professional - yolo26l-pose.pt 与 yolo26x-pose.pt（共约 184 MB）。
      All          - 以上全部权重。

.PARAMETER DestinationRoot
    目标目录。默认为 <包根目录>\models\pose，即 EchoPosture 默认查找的位置。

.PARAMETER Yes
    跳过交互式 Y/N 确认，直接执行。用于脚本化或 CI 场景；手动运行建议交互确认。

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\fetch_pose_models\fetch_pose_models_zh.ps1

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\fetch_pose_models\fetch_pose_models_zh.ps1 -Tier All
...
# 固定的上游权重标识，已于 2026-08-15 与 Ultralytics assets release API 核对。
# 哈希不一致即表示该文件不是经审计的权重。
$Catalog = @(
    [pscustomobject]@{
...
Write-Host ''
Write-Host 'EchoPosture 姿态模型下载器（官方源，中文版）' -ForegroundColor Cyan
Write-Host "  来源     : $BaseUri"
...
foreach ($item in $selected) {
    ...
    try {
        ...
        $failures += "$($item.Name)：下载失败 - $($_.Exception.Message)"
        Write-Host "[fail]    $($item.Name) 下载失败。" -ForegroundColor Red
        continue
    }
    ...
    if ($actualBytes -ne $item.Bytes -or $actualHash -ne $item.Sha256) {
        ...
        $failures += "$($item.Name)：完整性校验失败（实际 $actualBytes 字节，sha256 $actualHash）"
        Write-Host "[fail]    $($item.Name) 校验未通过，已丢弃。" -ForegroundColor Red
        continue
    }

    Move-Item -LiteralPath $temp -Destination $target -Force
    Write-Host "[ok]      $($item.Name) 校验通过。" -ForegroundColor Green
}
...
if ($failures.Count -gt 0) {
    ...
    Write-Host ''
    Write-Host '如果你的网络无法访问 GitHub，可尝试镜像版本：' -ForegroundColor Yellow
    Write-Host '  tools\fetch_pose_models\fetch_pose_models_mirror_zh.ps1' -ForegroundColor Yellow
    $exitCode = 1
} else {
