...
_TEXTS: Dict[str, Dict[str, str]] = {
    "zh": {
        ...
        "onb_mode_benchmarking": "正在测定显卡性能（{model}）",
        "onb_mode_failed_fallback": "该模式启动失败，已回退到兼容模式：{detail}",
        "onb_mode_failed_fallback_standard": "该模式启动失败，已回退到标准模式：{detail}",
        ...
        "vision_mode_professional_unavailable": "专业模式 Beta 不可用：本机不满足 CUDA 独立显卡要求。",
        "vision_mode_pro_unavailable_no_cuda_torch": "专业模式 Beta 不可用：运行时装的是 CPU 版 PyTorch，需要 CUDA 版本。",
        "vision_mode_pro_unavailable_no_driver": "专业模式 Beta 不可用：未检测到 NVIDIA 显卡驱动。",
        "vision_mode_pro_unavailable_no_weights": "专业模式 Beta 不可用：缺少本地 YOLO26l/x-pose 权重。",
        "vision_pro_active_notice": "专业模式 Beta · {model} · CUDA · 实测 {hz} Hz（P50 {p50} ms / P95 {p95} ms）",
        "vision_pro_active_warmup": "专业模式 Beta · {model} · CUDA · 正在采集实测帧率",
        "vision_pro_fallback_perf": "{model} 实测 P95 {p95} ms，未达到 {target} ms 门槛。",
        "vision_pro_fallback_oom": "{model} 显存不足，已降级到 {fallback}。",
    ...
    "en": {
        ...
        "onb_mode_benchmarking": "Measuring GPU performance ({model})",
        "onb_mode_failed_fallback": "This mode failed to start and fell back to Compatibility: {detail}",
        "onb_mode_failed_fallback_standard": "This mode failed to start and fell back to Standard: {detail}",
        ...
        "vision_mode_professional_unavailable": "Professional mode Beta unavailable: this machine does not meet the CUDA discrete-GPU requirement.",
        "vision_mode_pro_unavailable_no_cuda_torch": "Professional mode Beta unavailable: the runtime has CPU-only PyTorch; a CUDA build is required.",
        "vision_mode_pro_unavailable_no_driver": "Professional mode Beta unavailable: no NVIDIA display driver was detected.",
        "vision_mode_pro_unavailable_no_weights": "Professional mode Beta unavailable: the local YOLO26l/x-pose weights are missing.",
        "vision_pro_active_notice": "Professional Beta · {model} · CUDA · {hz} Hz measured (P50 {p50} ms / P95 {p95} ms)",
        "vision_pro_active_warmup": "Professional Beta · {model} · CUDA · measuring live frame rate",
        "vision_pro_fallback_perf": "{model} measured P95 {p95} ms, short of the {target} ms target.",
        "vision_pro_fallback_oom": "{model} ran out of GPU memory; stepped down to {fallback}.",
