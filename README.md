...
Windows 桌面端姿态监测原型。主入口会以系统托盘方式静默运行，用摄像头识别用户的脸部、肩膀和躯干关系，并在明确需要调整姿势时逐步降低屏幕可见亮度。当前 DEV 版不再启用截图蒙版或不可控的系统级实时模糊。
...
校准完成后，右下角系统托盘会出现 EchoPosture 小标识。右键托盘图标可以打开菜单，点击 `停止` 会退出程序，并清除所有视觉覆盖效果。双击托盘图标会打开一个小状态面板，只显示当前状态、压暗程度和模糊程度。
...
- 当前默认关闭不可控的系统级实时模糊，避免突然进入最大模糊；状态面板里的模糊程度会保持为 0%。
- 姿态恢复后约 0.3 秒内清除暗化效果；模糊程度在当前 DEV 版中保持为 0%。
...
- `dist\EchoPosture-DEV-20260530-204033-win-x64`
